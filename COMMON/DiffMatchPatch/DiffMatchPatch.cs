...
internal class diff_match_patch
{
    ...
    private readonly float Diff_Timeout = 1.0f;

    // Cost of an empty edit operation in terms of edit characters.
    private readonly short Diff_EditCost = 4;

    // At what point is no match declared (0.0 = perfection, 1.0 = very loose).
    private readonly float Match_Threshold = 0.5f;

    // How far to search for a match (0 = exact location, 1000+ = broad match).
    // A match this many characters away from the expected location will add
    // 1.0 to the score (0.0 is a perfect match).
    private readonly int Match_Distance = 1000;

    // When deleting a large block of text (over ~64 characters), how close
    // do the contents have to be to match the expected contents. (0.0 =
    // perfection, 1.0 = very loose).  Note that Match_Threshold controls
    // how closely the end points of a delete need to match.
    private readonly float Patch_DeleteThreshold = 0.5f;

    // Chunk size for context length.
    private readonly short Patch_Margin = 4;
     ...
     * @return List of Diff objects.
     */
    private List<Diff> diff_bisect(string text1, string text2,
        DateTime deadline)
    {
     ...
     */
    private object[] diff_linesToChars(string text1, string text2)
    {
     ...
     * @param lineArray List of unique strings.
     */
    private void diff_charsToLines(ICollection<Diff> diffs,
     ...
     * @param diffs List of Diff objects.
     */
    public List<Diff> diff_cleanupSemantic(List<Diff> diffs)
    {
        var changes = false;
     ...
     */
    private Dictionary<char, int> match_alphabet(string pattern)
    {
    ...
    #region EncodeURI +EncodeURI(string str)
